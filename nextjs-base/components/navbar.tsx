...
// import { CommandIcon, ShoppingCartIcon, GlobeIcon, GiftIcon, CreditCardIcon, TruckIcon, BarChart2Icon, ShieldCheckIcon, BoxesIcon, PlugIcon, BookOpenIcon, LandmarkIcon, UsersIcon, InfoIcon, BriefcaseIcon, LifeBuoyIcon, HelpCircleIcon, MessageSquareIcon, PrinterIcon, Code2Icon } from "lucide-react";
                ...
                {isSheet && (
                  <div className={`mt-2 space-y-2 ${isResourcesOpen ? 'block' : 'hidden'}`}>
                    {item.children.map((child) => {
                      ...
                      return (
                        <div key={child.title} className="pl-4">
                          <Link href={child.href} className="flex items-center gap-2 py-2 text-sm text-foreground hover:text-blue-300">
                            {Icon && <Icon className="w-4 h-4 text-blue-300 flex-shrink-0" />}
